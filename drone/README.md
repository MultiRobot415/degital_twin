...
- mocap driver の `config/mocap4r2_optitrack_driver_params.yaml` の `server_address`(MotivePCのIP)/`local_address`(このPCのIP) を実環境に。
...
cd ~/research/drone/ros2_ws && source install/setup.bash
ros2 launch drone_control bringup_real.launch.py rigid_body_id:=<MotiveのID> drone_ip:=192.168.11.51
```
